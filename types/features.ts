...
export interface Dungeon {
    ...
    timeLimit: number;
    minLevel: number;
    recommendedStats: StatType[]; // Added for compatibility
    rewards: {
        xp: number;
        items: string[];
...
// --- U30 DAILY QUESTS ---
export type QuestType = 'mission_completion' | 'stat_threshold' | 'dungeon_clear' | 'shadow_extract' | 'health_score' | 'streak_maintain' | 'manual_verification';
...
export interface QuestReward {
    ...
    shards?: number;
    buffId?: BuffId;
    stats?: Partial<UserStats>; // U40 - Stat Rewards
}
...
// --- REWARDS ---
export type RewardType = 'levelup' | 'item' | 'title' | 'frame' | 'currency' | 'season_rank' | 'milestone' | 'quest' | 'mission_complete' | 'achievement' | 'shadow';
...
export interface RewardItem {
    ...
    targetStat?: StatType;
    shards?: number;
    stats?: { stat: string; value: number }[]; // Equipment stat bonuses
}
