...
import { User, Zap, Trophy, Book, Calendar, Flame, X, Settings, Brain, Skull, DoorOpen, Ghost, Activity, ShoppingBag, Shield, Map, MessageSquare } from 'lucide-react';
...
export const GateMenu: React.FC<GateMenuProps> = ({ isOpen, onClose, onNavigate }) => {
  const tiles = [
    ...
    { id: 'Gear', label: 'Equipment', icon: Shield, color: 'orange', desc: 'Weapons & Armor' }, // U31
    { id: 'Shadows', label: 'Shadow Army', icon: Ghost, color: 'purple', desc: 'Manage Soldiers' },
    { id: 'Skills', label: 'Skills', icon: Zap, color: 'emerald', desc: 'Abilities' },
    ...
    { id: 'PowerAnalysis', label: 'Power Analysis', icon: Activity, color: 'indigo', desc: 'Source Breakdown' }, // U30
    { id: 'Shop', label: 'System Shop', icon: ShoppingBag, color: 'amber', desc: 'Exchange QP' }, // U30 Refactor
    { id: 'JobChange', label: 'Job Change', icon: Brain, color: 'purple', desc: 'Class Progression' }, // U35
    { id: 'DailyChat', label: 'Daily Chat', icon: MessageSquare, color: 'emerald', desc: 'Voice Journal' },
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#02040a]/95 backdrop-blur-xl p-4 pointer-events-auto">
      {/* Background Effects */}
      ...
      <div className="w-full max-w-md h-full flex flex-col relative z-10">
        ...
        <button
          onClick={onClose}
          className="absolute top-6 right-6 p-3 text-white bg-red-900/20 border border-red-500/30 hover:bg-red-900/40 hover:border-red-500 rounded-full transition-all z-50 shadow-[0_0_15px_rgba(220,38,38,0.3)]"
        >
          <X size={28} />
        </button>
        ...
        <div className="grid grid-cols-2 gap-4 overflow-y-auto pb-24 scrollbar-hide px-2">
          {tiles.map((tile, index) => {
            ...
            switch (tile.color) {
              ...
              case 'orange': colorStyles = "border-orange-500/30 hover:border-orange-400 shadow-[0_0_15px_rgba(249,115,22,0.1)] hover:shadow-[0_0_25px_rgba(249,115,22,0.3)] text-orange-400"; break;
              case 'emerald': colorStyles = "border-emerald-500/30 hover:border-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.1)] hover:shadow-[0_0_25px_rgba(16,185,129,0.3)] text-emerald-400"; break;
              case 'indigo': colorStyles = "border-indigo-500/30 hover:border-indigo-400 shadow-[0_0_15px_rgba(99,102,241,0.1)] hover:shadow-[0_0_25px_rgba(99,102,241,0.3)] text-indigo-400"; break;
              case 'violet': colorStyles = "border-violet-500/30 hover:border-violet-400 shadow-[0_0_15px_rgba(139,92,246,0.1)] hover:shadow-[0_0_25px_rgba(139,92,246,0.3)] text-violet-400"; break;
              case 'amber': colorStyles = "border-amber-500/30 hover:border-amber-400 shadow-[0_0_15px_rgba(245,158,11,0.1)] hover:shadow-[0_0_25px_rgba(245,158,11,0.3)] text-amber-400"; break;
            ...
            return (
              <button
                ...
                onClick={(e) => {
                  ...
                  onClose();
                  setTimeout(() => onNavigate(tile.id), 50);
                }}
                className={`
