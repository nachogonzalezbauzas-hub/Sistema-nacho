...
export const MainLayout: React.FC<MainLayoutProps & { isOnboarding?: boolean }> = ({
    children,
    currentTab,
    setCurrentTab,
    isGateOpen,
    ...
    return (
        <div className={`min-h-screen font-sans selection:bg-blue-500/30 overflow-x-hidden text-blue-50 ${state.settings.theme === 'dark' ? 'theme-dark' : 'theme-light'}`}>
            ...
            {state.rewardQueue.length > 0 && !isOnboarding && (
                <React.Fragment>
            ...
            {!isOnboarding && <DungeonFailureModal />}
            ...
            {/* --- MAIN CONTENT --- */}
            <main className={`max-w-xl mx-auto p-4 relative z-10 min-h-[calc(100vh-140px)] pb-32 ${isOnboarding ? 'h-screen pb-0' : ''}`}>
                {children}
            </main>
