        ...
        >
            ...
            <div className="relative z-10 flex flex-col items-center px-4">
                ...
                {phase === 'idle' && (
                    ...
                    >
                        {/* Levels Selector */}
                        {onLevelsChange && item.level < maxLevel && (
                            <div className="bg-slate-900/80 p-3 rounded-xl border border-slate-800">
                                <div className="text-xs text-slate-500 uppercase mb-2">Upgrade Amount</div>
                                <div className="flex gap-2">
                                    <button
                                        onClick={() => onLevelsChange(1)}
                                        className={`flex-1 py-2 rounded-lg font-bold text-sm transition-all ${levels === 1
                                    ...
                                    <button
                                        onClick={() => onLevelsChange(Math.min(5, maxLevel - item.level))}
                                        className={`flex-1 py-2 rounded-lg font-bold text-sm transition-all ${levels === Math.min(5, maxLevel - item.level) && levels !== (maxLevel - item.level)
                                            ? 'bg-blue-600 text-white shadow-lg'
                                            : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
                                        disabled={maxLevel - item.level < 2}
                                    >
                                        +5
                                    </button>
                                    {/* MAX Button */}
                                    <button
                                        onClick={() => {
                                            const levelsRemaining = maxLevel - item.level;
                                            // Calculate affordable levels
                                            let affordableLevels = 0;
                                            let accumulatedCost = 0;

                                            for (let i = 0; i < levelsRemaining; i++) {
                                                const lvlCost = 100 * (item.level + i + 1);
                                                if (accumulatedCost + lvlCost <= currentShards) {
                                                    accumulatedCost += lvlCost;
                                                    affordableLevels++;
                                                } else {
                                                    break;
                                                }
                                            ...
                                            // If we can afford 0, set to 1 (will be disabled by canAfford anyway)
                                            // If affordableLevels < levelsRemaining, use affordableLevels.
                                            // Make sure we select at least 1 if we have some shards, or just 1 to show cost.
                                            onLevelsChange(Math.max(1, affordableLevels));
                                        }}
                                        className={`flex-1 py-2 rounded-lg font-bold text-sm transition-all ${levels > 5 || levels === maxLevel - item.level
                                ...
                                <div className="text-[10px] text-slate-600 mt-2 text-center">
                                    +{item.level} → +{item.level + levels} / {maxLevel} max
                                </div>
                            </div>
                        )}
                        ...
                        <div className="flex justify-between text-sm font-mono bg-slate-900/80 p-3 rounded-xl border border-slate-800">
                            <span className="text-slate-400">Coste ({levels}x)</span>
