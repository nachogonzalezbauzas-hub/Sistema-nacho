...
import { Volume2, Music, Speaker, Moon, Sun, Check, Globe, Zap } from 'lucide-react';
...
export const GameSettings: React.FC<GameSettingsProps> = ({ settings, updateSettings }) => {
    ...
    return (
        <>
            ...
            <Card className="p-5 space-y-4 bg-slate-900/60 border-blue-900/30">
                <div className="flex items-center gap-3 mb-2">
                    <div className="p-2 rounded-lg bg-amber-950 border border-amber-900 text-amber-400">
                        <Zap size={20} />
                    </div>
                    <h3 className="text-sm font-bold text-slate-200 uppercase tracking-wider">Graphics</h3>
                </div>

                <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950/50 border border-slate-800/50">
                    <div className="flex items-center gap-3">
                        <Zap size={16} className={settings.performanceMode ? "text-amber-400" : "text-slate-600"} />
                        <div>
                            <div className="text-sm font-bold text-slate-300">Performance Mode</div>
                            <div className="text-[10px] text-slate-500">Reduces visual effects to save battery</div>
                        </div>
                    </div>
                    <button
                        ...
                        className={`
              relative w-11 h-6 rounded-full transition-colors duration-300
              ${settings.performanceMode ? 'bg-amber-600' : 'bg-slate-700'}
            `}
                    >
                        <div className={`
              absolute top-1 left-1 w-4 h-4 rounded-full bg-white transition-transform duration-300
