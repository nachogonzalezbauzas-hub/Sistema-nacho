...
export const DataSettings: React.FC<DataSettingsProps> = ({ redeemCode, resetAll, onForceRefreshQuests, language }) => {
    ...
    return (
        <>
            ...
            <Card className="p-5 space-y-4 bg-slate-900/60 border-blue-900/30 overflow-visible">
                ...
                {/* Debug: Force Refresh Quests (Temporary/Beta) */}
                <div className="mb-4 pb-4 border-b border-white/5">
                    <button
                        onClick={onForceRefreshQuests}
                        className="w-full py-2 bg-blue-900/30 border border-blue-500/30 rounded-lg text-blue-400 text-xs font-bold uppercase hover:bg-blue-900/50 transition-all flex items-center justify-center gap-2"
                    >
                        <span>🔄</span> {language === 'es' ? 'Forzar Reset Misiones Diarias' : 'Force Reset Daily Quests'}
                    </button>
                    <p className="text-[10px] text-slate-500 text-center mt-1 italic">
                        {language === 'es' ? '(Solo para pruebas: Regenera misiones)' : '(Testing only: Regenerates missions)'}
                    </p>
                </div>
