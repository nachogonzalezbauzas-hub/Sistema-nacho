...
export const ShadowCard: React.FC<ShadowCardProps> = ({ shadow, onEquip, language }) => {
                ...
                <div className="bg-slate-950/50 p-2 rounded-lg border border-slate-800/50 space-y-1">
                    <div className="flex justify-between items-center">
                        <span className="text-[10px] text-slate-500 uppercase font-bold">
                            {shadow.evolutionLevel === 0 ? 'Base' : shadow.evolutionLevel === 1 ? 'Elite' : 'Marshal'}
                        </span>
                        <span className="text-[10px] text-purple-400 font-mono">
                            {shadow.experiencePoints || 0} / {shadow.xpToNextEvolution || 500} XP
                        </span>
                    </div>
                    <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                        <div
                            className="h-full bg-gradient-to-r from-purple-600 to-fuchsia-500 transition-all duration-500"
                            style={{ width: `${Math.min(100, ((shadow.experiencePoints || 0) / (shadow.xpToNextEvolution || 500)) * 100)}%` }}
                        />
                    </div>
                    {shadow.evolutionLevel >= 2 && (
                        <div className="text-center text-[10px] text-fuchsia-400 font-bold animate-pulse">MAX EVOLUTION</div>
                    )}
                </div>
                ...
                >
                    {shadow.isEquipped ? (
                        <span className="flex items-center gap-2 justify-center text-purple-300 font-bold">
                            <Check size={14} /> {t('shadow_summoned', language)}
                        </span>
                    ) : (
                        <span className="group-hover:tracking-widest transition-all duration-300">{t('shadow_summon', language)}</span>
                    )}
                </Button>
            </div>
        </Card>
