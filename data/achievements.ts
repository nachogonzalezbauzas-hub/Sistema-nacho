...
export const ACHIEVEMENTS: Achievement[] = [
  // --- GENERAL (Premium Milestones) ---
  ...[10, 50, 100].map(createLevelAchievement),
  {
    id: 'first_login',
  ...
  ...[7, 30, 90, 365].map(days => ({
    ...
    icon: 'Flame',
    rarity: (days >= 365 ? 'MYTHIC' : days >= 90 ? 'EPIC' : days >= 30 ? 'RARE' : 'UNCOMMON') as AchievementRarity,
    condition: (state: AppState) => state.stats.streak >= days
  })),

  // --- COMBAT (Nerfed Targets for V2.1) ---
  ...[1, 10, 50, 100].map(createDungeonAchievement),
  ...[1, 10, 50].map(count => ({
    id: `boss_${count}`,
    category: 'COMBAT' as AchievementCategory,
    title: { en: `Boss Slayer ${count}`, es: `Matajefes ${count}` },
    description: { en: `Defeat ${count} Dungeon Bosses.`, es: `Derrota a ${count} Jefes de Mazmorra.` },
    icon: 'Skull',
    rarity: (count >= 50 ? 'LEGENDARY' : count >= 10 ? 'RARE' : 'COMMON') as AchievementRarity,
    condition: (state: AppState) => state.dungeonRuns.filter(r => r.victory && !!r.bossId).length >= count
  })),
  ...[10000, 50000, 100000, 250000, 500000].map(createPowerAchievement),
  ...
  ...[5, 15, 30, 50].map(count => ({
    ...
    icon: 'Users',
    rarity: (count >= 50 ? 'LEGENDARY' : count >= 30 ? 'EPIC' : count >= 15 ? 'RARE' : 'UNCOMMON') as AchievementRarity,
  ...
  ...['S', 'SS', 'SSS'].map(rank => ({
    ...
    rarity: (rank === 'SSS' ? 'LEGENDARY' : rank === 'SS' ? 'EPIC' : 'RARE') as AchievementRarity,
    condition: (state: AppState) => state.shadows.some(s => s.rank === rank)
  })),

  // --- GROWTH (v2.1 Compressed) ---
  ...[20, 50, 100].map(v => createStatAchievement('strength', v, 'Dumbbell')),
  ...[20, 50, 100].map(v => createStatAchievement('agility', v, 'Wind')),
  ...[20, 50, 100].map(v => createStatAchievement('vitality', v, 'Heart')),
  ...[20, 50, 100].map(v => createStatAchievement('intelligence', v, 'Brain')),
  ...[20, 50, 100].map(v => createStatAchievement('fortune', v, 'Sparkles')),
  ...[100, 250, 500].map(total => ({
    ...
    icon: 'TrendingUp',
    rarity: (total >= 500 ? 'LEGENDARY' : total >= 250 ? 'EPIC' : 'RARE') as AchievementRarity,
  ...
  ...[1000, 10000, 50000].map(count => ({
    ...
    icon: 'Sparkles',
    rarity: (count >= 50000 ? 'LEGENDARY' : count >= 10000 ? 'EPIC' : 'UNCOMMON') as AchievementRarity,
  ...
  ...[1, 5, 10].map(count => ({
    ...
    icon: 'Tag',
    rarity: (count >= 10 ? 'LEGENDARY' : count >= 5 ? 'EPIC' : 'COMMON') as AchievementRarity,
  ...
  ...[50, 100].map(count => ({
    ...
    icon: 'Calendar',
    rarity: (count >= 100 ? 'EPIC' : 'RARE') as AchievementRarity,
  ...
  ...[50, 200, 500].map(count => ({
    ...
    icon: 'CheckCircle',
    rarity: (count >= 500 ? 'LEGENDARY' : count >= 200 ? 'EPIC' : 'UNCOMMON') as AchievementRarity,
    condition: (state: AppState) => state.missions.filter(m => m.lastCompletedAt).length >= count
  }))
];
