...
import { MissionRewardSummary } from '../rewards/RewardModal';
import { UnifiedRewardOverlay } from '../ui/UnifiedRewardOverlay';
import { GateMenu } from './GateMenu';
import { RewardModal } from '../rewards/RewardModal';
import { DungeonRunResultView } from '../../views/DungeonRunResultView';
import { PlayerRankTitle } from '../ui/PlayerRankTitle';
...
export type Tab = 'Dashboard' | 'Misiones' | 'Logs' | 'Físico' | 'Logros' | 'Perfil' | 'Skills' | 'Calendar' | 'Buffs' | 'Season' | 'Settings' | 'Shadows' | 'PowerAnalysis' | 'Shop' | 'Gear' | 'Dungeons' | 'JobChange';
...
interface MainLayoutProps {
    ...
    setIsRewardOpen: (isOpen: boolean) => void;
...
export const MainLayout: React.FC<MainLayoutProps> = ({
    ...
    return (
        <div className={`min-h-screen font-sans selection:bg-blue-500/30 overflow-x-hidden text-blue-50 ${state.settings.theme === 'dark' ? 'theme-dark' : 'theme-light'}`}>
            ...
            {/* Dungeon Result Overlay (Global) */}
            {state.isDungeonResultVisible && <DungeonRunResultView />}
            ...
            <header className="sticky top-0 z-40 h-16 px-4 flex items-center justify-between">
                ...
                <div className="relative z-10 w-full max-w-xl mx-auto flex items-center justify-between">
                    ...
                    {/* Left: Rank Title */}
                    <PlayerRankTitle level={state.stats.level} />
                    ...
                    <div className="flex items-center gap-1.5 bg-black/60 border border-purple-500/30 px-3 py-1 rounded-full backdrop-blur-md shadow-[0_0_15px_rgba(168,85,247,0.2)]">
                        ...
                        <span className="text-xs font-black text-white tracking-wide">{state.shards || 0}</span>
                    </div>
                </div>
            </header>
            ...
            <nav className="fixed bottom-0 left-0 right-0 z-40 pb-safe">
                ...
                <div className="relative z-10 max-w-xl mx-auto flex justify-around items-center h-20 px-2">
                    ...
                    <NavButton active={currentTab === 'Misiones'} onClick={() => setCurrentTab('Misiones')} icon={<Target size={22} />} label="MISSIONS" />
