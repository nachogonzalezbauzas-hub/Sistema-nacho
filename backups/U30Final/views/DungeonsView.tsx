...
export const DungeonsView: React.FC<DungeonsViewProps> = ({ state }) => {
    ...
    const completedDungeonIds = new Set((state.dungeonRuns || []).filter(r => r.victory).map(r => r.dungeonId));

    const getIsLocked = (index: number) => {
        if (index === 0) return false;
        const prevDungeonId = ALL_DUNGEONS[index - 1].id;
        return !completedDungeonIds.has(prevDungeonId);
    };
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-3xl font-black text-white tracking-tighter uppercase italic">
                        Gate <span className="text-blue-500">System</span>
                    </h2>
                    <p className="text-gray-400 text-sm">Conquer dungeons to increase your power.</p>
                </div>
                <div className="bg-blue-900/20 border border-blue-500/30 px-4 py-2 rounded-xl flex items-center gap-2">
                    ...
                    <span className="text-xs text-blue-400 uppercase tracking-wider ml-1">CP</span>
                </div>
            </div>
                    ...
                    {ALL_DUNGEONS.map((dungeon, index) => {
                        ...
                        return (
                            <motion.div
                                ...
                                onClick={() => !isLocked && setSelectedDungeonId(dungeon.id)}
                                className={`
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                                                <span>Lvl {dungeon.recommendedLevel}</span>
                                                <span>•</span>
                                                <span className={userPower >= dungeon.recommendedPower ? 'text-green-400' : 'text-red-400'}>
                                                    Req: {dungeon.recommendedPower.toLocaleString()} CP
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <AnimatePresence mode="wait">
                        {selectedDungeon ? (
                            ...
                            >
                                ...
                                <div className="space-y-4 mb-8">
                                    <div className="bg-black/40 p-4 rounded-xl border border-gray-800">
                                        <div className="flex justify-between items-center mb-2">
                                            <span className="text-gray-400 text-xs uppercase tracking-wider">Recommended Power</span>
                                        ...
                                        <div className="mt-2 text-right">
                                            {userPower >= selectedDungeon.recommendedPower ? (
                                                <span className="text-green-400 text-xs font-bold flex items-center justify-end gap-1">
                                                    <Zap size={12} /> VICTORY GUARANTEED
                                                </span>
                                            ) : (
                                                <span className="text-red-400 text-xs font-bold flex items-center justify-end gap-1">
                                                    <AlertTriangle size={12} /> DANGER: HIGH CHANCE OF FAILURE
                                                </span>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {selectedDungeon.boss && (
                                        <div className="bg-purple-900/10 border border-purple-500/30 p-4 rounded-xl">
                                            <p className="text-purple-400 text-xs uppercase tracking-widest mb-2">Boss Reward</p>
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-xs text-purple-400">Guaranteed Extraction</div>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                ...
                                <button
                                    onClick={() => startDungeonRun(selectedDungeon.id)}
                                    ...
                                    `}
                                >
                                    {userPower >= selectedDungeon.recommendedPower ? 'ENTER DUNGEON' : 'INSUFFICIENT POWER'}
                                </button>
                            </motion.div>
                        ) : (
