...
interface GearViewProps {
    ...
    onUpgrade: (id: string, onSuccess?: () => void, onFailure?: () => void) => void;
...
export const GearView: React.FC<GearViewProps> = ({ state, onEquip, onUnequip, onUpgrade, onSalvage, onAddEquipment, language = 'en' as Language }) => {
    ...
    const handleUpgradeClick = (id: string) => {
        ...
        if (item) setUpgradeModalItem(item);
    };

    const handleUpgradeConfirm = (id: string) => {
        setIsUpgrading(true);
            ...
            <AnimatePresence>
                {upgradeModalItem && (
                    <UpgradeModal
                        ...
                        onConfirm={() => handleUpgradeConfirm(upgradeModalItem.id)}
                        cost={100 * (upgradeModalItem.level + 1)}
                        canAfford={state.shards >= 100 * (upgradeModalItem.level + 1)}
                        successChance={(() => {
