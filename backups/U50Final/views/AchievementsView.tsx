...
import { ACHIEVEMENTS, getUnlockedAchievements, saveUnlockedAchievement, AchievementCategory, Achievement } from '@/data/achievements';
import { TITLES, BASE_FRAMES } from '@/data/titles';
import { SHOP_FRAMES } from '@/data/shopCosmetics';
import {
...
export const AchievementsView: React.FC<AchievementsViewProps> = ({ state, onEquipTitle, language }) => {
  ...
  useEffect(() => {
    ...
    // Check for new unlocks in a batch
  ...
  // Weights for sorting: title rarities AND frame ranks (both lowercase for comparison)
  const RARITY_WEIGHT: Record<string, number> = {
    // Title Rarities (lowercase)
    celestial: 7,
    godlike: 6,
    mythic: 5,
    legendary: 4,
    ...
    uncommon: 1,
    common: 0,
    // Frame Ranks (lowercase for comparison)
    sss: 6,
    ss: 5,
    s: 4,
    a: 3,
  ...
  const sortItems = <T extends { rarity: string; name: string }>(items: T[]) => {
    return [...items].sort((a, b) => {
      ...
      if (weightA !== weightB) return weightA - weightB; // Ascending rarity (Bad -> Best)
      return a.name.localeCompare(b.name); // Ascending name
    });
  };

  const DISPLAY_TITLES = sortItems([...TITLES, ...customTitles]);
