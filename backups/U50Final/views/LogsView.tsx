...
import { Card, LogCategoryBadge } from '@/components';
...
export const LogsView: React.FC<LogsViewProps> = ({ logs, stats, activeBuffs, language }) => {
  ...
  const groupedLogs = useMemo(() => {
    ...
    filteredLogs.forEach(log => {
      ...
      if (date.toDateString() === today.toDateString()) key = t('logs_today', language);
      else if (date.toDateString() === yesterday.toDateString()) key = t('logs_yesterday', language);
  ...
  const todayStats = useMemo(() => {
    ...
    return {
      ...
      statsGained: stats.dailyGains ? Object.values(stats.dailyGains).reduce((a, b) => (a as number) + (b as number), 0) : 0,
    };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="relative overflow-hidden rounded-2xl border border-blue-500/30 bg-slate-900/40 backdrop-blur-md shadow-[0_0_30px_rgba(37,99,235,0.15)] group">
            ...
            <div className="relative p-5">
              <h3 className="text-xs font-bold text-blue-300 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                ...
                {t('logs_daily_analysis', language)}
              </h3>
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                <div className="flex flex-col">
                  ...
                  <span className="text-[9px] text-blue-400/70 font-bold uppercase tracking-wider">{t('logs_xp_gained', language)}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-3xl font-black text-white drop-shadow-sm tabular-nums tracking-tight">
                    {todayStats.missions}
                  </span>
                  <span className="text-[9px] text-blue-400/70 font-bold uppercase tracking-wider">{t('logs_missions', language)}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-3xl font-black text-white drop-shadow-sm tabular-nums tracking-tight">
                    {todayStats.statsGained}
                  </span>
                  <span className="text-[9px] text-blue-400/70 font-bold uppercase tracking-wider">{t('logs_stats_up', language)}</span>
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-blue-500/20">
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-950/30 border border-blue-500/20">
                  ...
                  <span className="text-xs font-bold text-slate-300">
                    <span className="text-white">{activeBuffs.length}</span> {t('logs_buffs', language)}
                  </span>
                </div>
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-950/30 border border-blue-500/20">
                  <Flame size={14} className="text-orange-500" />
                  <span className="text-xs font-bold text-slate-300">
                    <span className="text-white">{stats.streak}</span> {t('logs_day_streak', language)}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-blue-900/40 bg-slate-950/50 p-5 backdrop-blur-sm">
            <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4 flex justify-between">
              <span>{t('logs_7_day_activity', language)}</span>
              <span className="text-blue-500/60">{t('logs_xp_trend', language)}</span>
            </h3>
        ...
        <div className="space-y-4 h-full flex flex-col">
          ...
          <div className="flex flex-col gap-3">
            <div className="relative group">
              <input
                ...
                placeholder={t('logs_search_placeholder', language)}
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-hide">
              <FilterPill cat="All" label={t('logs_filter_all', language)} />
              <FilterPill cat="Mission" label={t('logs_filter_missions', language)} />
              <FilterPill cat="LevelUp" label={t('logs_filter_levelup', language)} />
              <FilterPill cat="Health" label={t('logs_filter_health', language)} />
              <FilterPill cat="Streak" label={t('logs_filter_streak', language)} />
              <FilterPill cat="Achievement" label={t('logs_filter_awards', language)} />
              <FilterPill cat="Milestone" label={t('logs_filter_milestones', language)} />
              <FilterPill cat="System" label={t('logs_filter_system', language)} />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto pr-1 space-y-6 min-h-[400px] scrollbar-thin scrollbar-thumb-blue-900/50 scrollbar-track-transparent">
            {Object.entries(groupedLogs).map(([dateLabel, groupLogs]: [string, LogEntry[]]) => (
            ...
            {filteredLogs.length === 0 && (
              <div className="flex flex-col items-center justify-center py-20 opacity-50">
                ...
                <p className="text-slate-500 font-bold uppercase tracking-wider text-xs">{t('logs_no_logs', language)}</p>
              </div>
            )}
          </div>
