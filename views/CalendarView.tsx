...
import { CalendarHeader, CalendarDay, Card, StatIcon, StatBadge, LogCategoryBadge } from '@/components';
...
import { X, Trophy, Activity, Book, Flame, Calendar as CalendarIcon, CheckCircle, Circle } from 'lucide-react';
...
export const CalendarView: React.FC<CalendarViewProps> = ({ language }) => {
    const { getCalendarData, getDayInfo, state, toggleUserEvent, removeUserEvent } = useStore();
    ...
    const rawDayData = selectedDate ? getDayInfo(selectedDate) : null;
    const selectedDayData = rawDayData || (selectedDate ? {
        date: selectedDate,
        missions: [],
        xpGained: 0,
        streak: 0,
        ...
        journal: [],
        health: undefined
    } as CalendarDayData : null);

    // Filter user events for the selected day
    const selectedDayUserEvents = useMemo(() => {
        if (!selectedDate) return [];
        return state.userEvents.filter(e => isSameDay(new Date(e.date), parseISO(selectedDate)));
    }, [selectedDate, state.userEvents]);
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        {days.map(day => {
                            ...
                            const hasUserEvent = state.userEvents.some(e => isSameDay(new Date(e.date), day));
                ...
                {selectedDate && selectedDayData && (
                    ...
                    >
                        ...
                        <div className="p-6 space-y-6 pb-24">
                            ...
                            {/* User Events Section */}
                            {selectedDayUserEvents.length > 0 && (
                                <div className="space-y-3">
                                    <div className="flex items-center gap-2">
                                        <CalendarIcon size={16} className="text-green-400" />
                                        <h3 className="text-sm font-bold text-green-200 uppercase tracking-wider">Scheduled Events</h3>
                                    </div>
                                    <div className="space-y-2">
                                        {selectedDayUserEvents.map(event => (
                                            <div key={event.id} className="bg-slate-900/50 p-3 rounded-lg border border-green-900/30 flex items-center justify-between group">
                                                <div className="flex items-center gap-3">
                                                    <button onClick={() => toggleUserEvent(event.id)} className="text-slate-500 hover:text-green-400 transition-colors">
                                                        {event.isCompleted ? <CheckCircle size={18} className="text-green-500" /> : <Circle size={18} />}
                                                    </button>
                                                    <div className={event.isCompleted ? 'line-through opacity-50' : ''}>
                                                        <p className="text-sm text-white font-medium">{event.title}</p>
                                                        <p className="text-xs text-slate-500 font-mono">{format(new Date(event.date), 'HH:mm')}</p>
                                                    </div>
                                                </div>
                                                <button onClick={() => removeUserEvent(event.id)} className="opacity-0 group-hover:opacity-100 p-1 text-red-400 hover:bg-red-900/20 rounded transition-all">
                                                    <X size={14} />
                                                </button>
                                            </div>
                                        ))}
