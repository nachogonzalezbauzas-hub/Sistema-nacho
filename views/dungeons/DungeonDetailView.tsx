...
import { Card, Button, StatIcon } from '@/components';
    ...
    return (
        <div className="h-full flex flex-col pb-24 animate-in fade-in zoom-in-95 duration-300">
                    ...
                    <div className="flex justify-center gap-1">
                        {[...Array(5)].map((_, i) => (
                            <motion.div
                                ...
                                className={`w-2 rounded-sm ${i < (typeof dungeon.difficulty === 'number' ? dungeon.difficulty : 1) ? 'bg-purple-500 shadow-[0_0_10px_#a855f7]' : 'bg-slate-800'}`}
                            />
                        ))}
                    </div>
                    <p className="text-purple-400 font-bold uppercase tracking-widest text-xs">Difficulty Rank {dungeon.difficulty}</p>
                </div>
                ...
                <div>
                    ...
                    <div className="flex flex-wrap justify-center gap-2">
                        <div className="px-3 py-1.5 rounded bg-blue-900/20 border border-blue-500/30 text-blue-400 text-xs font-bold">
                            {Math.floor(dungeon.rewards.xp * 0.4)} XP
                        </div>
                        {dungeon.boss && dungeon.boss.canExtract && (
                            <div className="px-3 py-1.5 rounded bg-purple-900/20 border border-purple-500/30 text-purple-400 text-xs font-bold">
                                Shadow Extraction
                            </div>
                        )}
                        {/* Generic Items */}
            ...
            <div className="p-4 bg-slate-950/80 backdrop-blur-md border-t border-white/10">
                <Button
                    onClick={onEnter}
                    className="w-full h-14 text-lg font-black tracking-widest bg-purple-600 hover:bg-purple-500 shadow-[0_0_30px_rgba(168,85,247,0.4)] animate-pulse-slow active:scale-95 transition-transform touch-manipulation"
                >
